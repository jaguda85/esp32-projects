...
//SPIClass vspi(VSPI);
SPIClass hspi(HSPI);
SPIClass spiBus(FSPI);  // Only one SPI bus on ESP32-C3
// radio(CE, CS)
...
void initRadios() {
  ...
  if (radio.begin(&spiBus)) {
    ...
    display.println("Radio 1: OK");  // Display success message
    ...
    display.println("Radio 1: FAIL");  // Display failure message
  }

  // Initialize Radio 2
  if (radio2.begin(&spiBus)) {
    Serial.println("Radio 2 Started");
    display.println("Radio 2: OK");  // Display success message
    ...
    display.println("Radio 2: FAIL");  // Display failure message
  }

  display.display();  // Update the screen with new text
}
void testRadios() {
  // Ensure proper SPI pin configuration for VSPI
  //vspi.begin();
...
void drawMenu() {
  ...
  const char *menuLabels[NUM_MENU_ITEMS] = {
    "BT Jammer", "Drone Jammer", "Wifi Jammer", "Multi Ch Jam", "Test Radios", "Settings", "Help"
  };
...
void executeSelectedMenuItem() {
  switch (selectedMenuItem) {
    ...
    case TEST_RADIOS:
      ...
      Serial.println("TEST RADIOS button pressed");
      displayInfo("TEST RADIOS", "TESTING RADIOS", "Testing.......");
      testRadios();
      nonBlockingDelay(2000);  // Debounce nonBlockingDelay
      break;
  }
